 ...
 * - Telegram Bot API для отправки голосовых сообщений
 * 
...
export class BotVoiceService {
  ...
  /**
   * 🎵 Оптимизация аудио для Telegram Bot API
   */
  async optimizeForTelegramBot(audioBuffer: Buffer): Promise<Buffer> {
  ...
  async generateEmotionalVoice(text: string, emotion: 'friendly' | 'professional' | 'enthusiastic' | 'calm'): Promise<Buffer> {
    ...
    let audioBuffer = await this.textToSpeech(text, voiceSettings);
    audioBuffer = await this.optimizeForTelegramBot(audioBuffer);
    return audioBuffer;
  }
  ...
  async createTempAudioFile(audioBuffer: Buffer, filename?: string): Promise<string> {
    const tempFilename = filename || `voice_${Date.now()}.mp3`;
