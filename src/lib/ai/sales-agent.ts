...
- "Наставник по стрижкам" - только для опытных мастеров, практика на манекенах в три круга
...
export class SalesAgent {
  async classifyCustomer(message: string, context: CustomerContext): Promise<string> {
    const keywords = {
      ...
      experienced: ['опыт', 'работаю', 'лет в профессии', 'мастер', 'салон', 'клиенты'],
  ...
  shouldAskClarifyingQuestions(message: string, conversationHistory: any[]): boolean {
    ...
    if (conversationHistory.length === 0) {
      const shortRequests = [
        ...
        'стрижки', 'цвет волос', 'обучение', 'хочу учиться'
      ];
...
Расскажите все, что посчитаете нужным.`;
  ...
  async findRelevantCourses(message: string, classification?: string) {
    ...
    // Фильтруем курсы для новичков - не показываем продвинутые
    if (classification === 'beginner') {
      ...
      // Для НЕ новичков тоже фильтруем слишком простые курсы в общих запросах
      // Но только если не спрашивают конкретно
