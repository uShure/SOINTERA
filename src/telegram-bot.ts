        ...
        if (result.voiceResponse) {
          try {
            ...
            try {
              ...
              log('INFO', '✅ Голосовой ответ отправлен (sendVoice)');
            } catch (voiceError) {
              log('WARN', 'Не удалось отправить как голосовое сообщение, пробуем как аудио:', voiceError.message);
              
              // Если не получилось, пробуем отправить как аудио файл
              try {
                ...
                log('INFO', '✅ Голосовой ответ отправлен (sendAudio)');
              } catch (audioError) {
                log('ERROR', 'Не удалось отправить голосовой ответ:', audioError.message);
                // Отправляем текстовое сообщение о том, что голосовой ответ недоступен
                await this.bot.sendMessage(chatId, '🎤 К сожалению, не удалось отправить голосовой ответ. Вот текстовый ответ выше.');
              }
            }
            ...
            log('ERROR', 'Ошибка обработки голосового ответа:', error);
          }
        }
      } else {
